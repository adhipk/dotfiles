...
assert_contains "$GHOSTTY_CONFIG" "keybind = cmd+b=text:\\\\x01\\\\x62" "Ghostty maps Cmd+B to the tmux Yazi side-pane binding"
assert_contains "$GHOSTTY_CONFIG" "keybind = cmd+shift+b=text:\\\\x01\\\\x42" "Ghostty maps Cmd+Shift+B to the full Yazi window binding"
assert_not_contains "$GHOSTTY_CONFIG" "keybind = cmd+b=text:\\\\x01\\\\x7a" "Ghostty no longer maps Cmd+B to tmux pane zoom"
...
assert_not_contains "$DOTFILES_DIR/home/dot_tmux.conf" "C-[012] select-window" "Ctrl+0/1/2 are type selectors instead of fixed indices"
assert_contains "$DOTFILES_DIR/home/dot_tmux.conf" "C-3 select-window -t :3" "Ctrl+3 retains direct index switching"
