...
assert_contains "$DOTFILES_DIR/home/dot_config/git/ignore" '^todo\.txt$' "global Git ignore excludes legacy repo-local todo ledgers"
assert_contains "$DOTFILES_DIR/home/dot_config/git/ignore" '^\.agent-write-lock/$' "global Git ignore excludes legacy agent lock directories"
assert_file_exists "$DOTFILES_DIR/home/dot_agents/AGENTS.md.tmpl" "shared agent guidance is declared"
assert_file_exists "$DOTFILES_DIR/home/dot_agents/skills/todo/SKILL.md" "global todo orchestration skill is declared"
assert_file_exists "$DOTFILES_DIR/home/dot_agents/skills/todo/agents/openai.yaml" "global todo skill UI metadata is declared"
assert_file_exists "$DOTFILES_DIR/home/dot_agents/skills/todo/references/task-contract.md" "global todo task contract is declared"
assert_file_exists "$DOTFILES_DIR/home/bin/executable_daemon.tmpl" "daemon command bridge is declared"
...
assert_contains "$DOTFILES_DIR/home/dot_agents/AGENTS.md.tmpl" 'todo ls "repo:[$]repo_root" --json' "shared agent guidance scopes canonical task inspection to the active repository"
assert_contains "$DOTFILES_DIR/home/dot_agents/AGENTS.md.tmpl" 'select(.done == false)' "shared agent guidance excludes completed tasks from project context"
assert_contains "$DOTFILES_DIR/home/dot_agents/AGENTS.md.tmpl" 'Do not load the unfiltered machine-wide queue' "shared agent guidance keeps unrelated repositories out of project context"
assert_contains "$DOTFILES_DIR/home/dot_agents/AGENTS.md.tmpl" '~/.agents/tasks/todo.txt' "shared agent guidance uses the machine-wide task store"
assert_contains "$DOTFILES_DIR/home/dot_agents/AGENTS.md.tmpl" 'user request beginning with `@todo`' "shared agent guidance routes @todo to orchestration"
...
assert_contains "$DOTFILES_DIR/home/dot_config/opencode/opencode.json" "machine-wide agent queue" "OpenCode personal agent uses the shared task store"
assert_contains "$DOTFILES_DIR/home/dot_config/opencode/opencode.json" "@todo requests as dependency-aware multi-agent workflows" "OpenCode understands global todo orchestration"
...
assert_contains "$DOTFILES_DIR/home/dot_config/private_karabiner/karabiner.json" '"from_event": true' "Vim leader cancellation passes unrelated input through"
...
assert_contains "$DOTFILES_DIR/install.sh" 'export PATH=.*brew_prefix/bin' "installer prioritizes Homebrew runtimes before dependency controllers"
assert_contains "$DOTFILES_DIR/install.sh" "python3 -c 'import tomllib'" "installer preflights the Python TOML runtime"
...
assert_contains "$DOTFILES_DIR/bootstrap.sh" 'activate_homebrew_path' "bootstrap refreshes the Homebrew executable path"
...
assert_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "typeset -U path fpath" "zsh deduplicates path and fpath"
assert_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "Some tools assign PATH directly" "zsh deduplicates path after tool initialization"
assert_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" '\[ -r "$HOME/.config/zsh/zshrc.commands" \]' "zsh guards command helper sourcing"
assert_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "ANTIGEN_ZSH" "zsh resolves Antigen before sourcing it"
assert_line_order "$DOTFILES_DIR/home/dot_zshrc.tmpl" "antigen bundle zsh-users/zsh-autosuggestions" "antigen bundle zsh-users/zsh-syntax-highlighting" "zsh-syntax-highlighting loads after autosuggestions"
assert_line_order "$DOTFILES_DIR/home/dot_zshrc.tmpl" "antigen bundle zsh-users/zsh-completions" "antigen bundle zsh-users/zsh-syntax-highlighting" "zsh-syntax-highlighting loads after completions"
assert_line_order "$DOTFILES_DIR/home/dot_zshrc.tmpl" "antigen bundle zsh-users/zsh-history-substring-search" "antigen bundle zsh-users/zsh-syntax-highlighting" "zsh-syntax-highlighting loads after history substring search"
assert_not_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "^source zsh-syntax-highlighting.zsh" "zsh does not manually source syntax highlighting after Antigen"
assert_not_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "^source zsh-history-substring-search.zsh" "zsh does not manually source history substring search after Antigen"
assert_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "STARSHIP_CONFIG" "zsh sets a Starship config"
assert_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "starship init zsh" "zsh initializes Starship"
assert_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "command -v starship" "zsh guards Starship initialization"
assert_contains "$DOTFILES_DIR/home/dot_config/starship.toml" "config-schema.json" "Starship config declares its schema"
assert_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "command -v mise" "zsh guards mise initialization"
assert_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "command -v zoxide" "zsh guards zoxide initialization"
assert_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" '\[ -x /opt/homebrew/bin/terraform \]' "zsh guards Terraform completion"
assert_not_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "_zoxide_z_complete" "zsh uses zoxide's default completion"
assert_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "bindkey '\\\\ef' forward-word" "Option+Right preserves forward-word"
assert_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "bindkey '\\\\eb' backward-word" "Option+Left preserves backward-word"
assert_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "bindkey '\\\\e\\[1;3C' forward-word" "Option+Right CSI sequence is bound"
assert_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "bindkey '\\\\e\\[1;3D' backward-word" "Option+Left CSI sequence is bound"
assert_not_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "bindkey -s '\\\\ef'" "Alt+F does not override Option+Right"
assert_not_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "catppuccin" "zsh does not reference Catppuccin"
assert_not_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" "colorschemes" "zsh does not source shared colorschemes"
...
TERMINAL_WINDOW_TYPES_AWRIT_MIGRATION="$TERMINAL_WINDOW_TYPES_MODULE/install/remove-legacy-awrit-link.sh.tmpl"
...
FOCUS_OPEN_TMUX_WINDOW_COMMAND="$APP_FOCUS_MODULE/bin/focus-open-tmux-window.sh"
FOCUS_OPEN_TMUX_WINDOW_BRIDGE="$DOTFILES_DIR/home/dot_config/skhd/executable_focus-open-tmux-window.sh.tmpl"
...
assert_not_contains "$TERMINAL_WINDOW_TYPES_TMUX" "allow-passthrough" "Tmux window types leave graphics passthrough unset"
...
for file in \
    ...
    executable_daemon.tmpl \
    executable_tmux-border-accent.tmpl \
    executable_tmux-command-palette \
    executable_tmux-hub.tmpl \
...
assert_file_exists "$TERMINAL_WINDOW_TYPES_AWRIT_MIGRATION" "terminal-window-types owns the legacy Awrit cleanup migration"
assert_contains "$DOTFILES_DIR/home/.chezmoiscripts/run_onchange_before_remove-legacy-awrit-link.sh.tmpl" 'includeTemplate "../modules/terminal-window-types/install/remove-legacy-awrit-link.sh.tmpl"' "ChezMoi mounts the module-owned legacy Awrit cleanup migration"
assert_contains "$DOTFILES_DIR/home/.chezmoiscripts/run_onchange_before_remove-legacy-awrit-link.sh.tmpl" 'if .modules.terminalWindowTypes.enabled' "legacy Awrit cleanup bridge is safe after disabled module removal"
assert_contains "$TERMINAL_WINDOW_TYPES_AWRIT_MIGRATION" '\[\[ -L "\$legacy_link" \]\]' "legacy Awrit cleanup ignores regular files"
assert_contains "$TERMINAL_WINDOW_TYPES_AWRIT_MIGRATION" '[[ "$current_target" != "$legacy_target" ]]' "legacy Awrit cleanup verifies the former managed target"
assert_contains "$TERMINAL_WINDOW_TYPES_AWRIT_MIGRATION" 'unlink "$legacy_link"' "legacy Awrit cleanup unlinks only the verified launcher"
assert_not_contains "$TERMINAL_WINDOW_TYPES_AWRIT_MIGRATION" 'rm -' "legacy Awrit cleanup cannot remove the preserved checkout"
...
assert_file_exists "$FOCUS_OPEN_TMUX_WINDOW_COMMAND" "app-focus owns open tmux window slot focus"
assert_file_exists "$FOCUS_OPEN_TMUX_WINDOW_BRIDGE" "open tmux window slot helper bridge is declared"
assert_contains "$FOCUS_OPEN_TMUX_WINDOW_BRIDGE" 'includeTemplate "../modules/app-focus/bin/focus-open-tmux-window.sh"' "open tmux window slot helper uses a thin module bridge"
...
assert_contains "$DOTFILES_DIR/modules/agent-timer/install/prepare-disable.sh.tmpl" 'if not \.modules\.agentTimer\.enabled' "timer lifecycle declares the module-disable branch"
...
assert_contains "$DOTFILES_DIR/modules/agent-timer/install/prepare-disable.sh.tmpl" 'shutdown --reason auto-start-disabled --automatic-only' "disabling automatic timing retires only legacy automatic workers"
assert_contains "$DOTFILES_DIR/modules/agent-timer/install/manage-cron.sh.tmpl" '[$]timer.*enable' "enabled apply clears the transient shutdown latch after reconciliation"
assert_contains "$DOTFILES_DIR/modules/agent-timer/install/manage-cron.sh.tmpl" 'if and \.modules\.agentTimer\.enabled \.modules\.agentTimer\.autoStart' "cron backstop exists only for automatic timing"
assert_contains "$DOTFILES_DIR/modules/agent-timer/module.yaml" 'restore-after-auto-start-disable' "timer manifest declares its transient lifecycle marker"
...
assert_contains "$DOTFILES_DIR/home/.chezmoidata/10-modules.yaml" 'autoStart: false' "automatic agent timing is disabled by default"
assert_contains "$DOTFILES_DIR/modules/agent-timer/config/config.toml.tmpl" 'auto_start =.*autoStart' "timer TOML exposes the automatic-start switch"
assert_contains "$DOTFILES_DIR/modules/agent-timer/policy/time-boxed-delivery.md" 'missing command.*ordinary untimed agent behavior' "global policy makes unavailable timers fail open"
...
assert_contains "$DOTFILES_DIR/modules/agent-timer/policy/time-boxed-delivery.md" 'bounded detached read-only Codex process' "global policy keeps automated checkpoints outside the TUI"
assert_contains "$DOTFILES_DIR/modules/agent-timer/hooks/codex-hooks.json" 'cat >\/dev\/null' "cached Codex hooks fail open when the timer command is missing"
assert_not_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" 'agent-timer/env.zsh' "zsh does not source timer configuration"
...
assert_contains "$DOTFILES_DIR/modules/agent-timer/bin/agent-timer" '--ask-for-approval never exec --ephemeral --ignore-user-config --sandbox read-only' "agent timer uses a locked-down detached Codex report"
assert_contains "$DOTFILES_DIR/home/.chezmoidata/10-modules.yaml" 'reportTimeoutSeconds: 120' "detached Codex report timeout is pinned to 120 seconds"
assert_contains "$DOTFILES_DIR/modules/agent-timer/bin/agent-timer" 'AGENT_TIMER_AUTO_START=false' "detached Codex cannot recursively arm the timer"
assert_not_contains "$DOTFILES_DIR/modules/agent-timer/bin/agent-timer" 'send-keys' "automated timer never injects terminal keys"
assert_contains "$DOTFILES_DIR/modules/agent-timer/bin/agent-timer" 'warningDeliveryMode="native-notification"' "agent timer warnings never occupy terminal input"
...
assert_not_contains "$DOTFILES_DIR/home/dot_zshrc.tmpl" '^export TODO_\|^export DONE_FILE' "Shell does not override the wrapper-owned shared task directory"
...
assert_contains "$DOTFILES_DIR/modules/todo/module.yaml" '~/.agents/tasks/todo.txt' "todo module preserves the global ledger"
assert_contains "$DOTFILES_DIR/modules/todo/module.yaml" '~/.agents/tasks/handoffs' "todo module preserves agent handoffs"
assert_contains "$DOTFILES_DIR/modules/todo/module.yaml" '~/.agents/tasks/.agent-write-lock' "todo module declares its shared transient lock"
...
assert_file_exists "$TMUX_SESSIONS_MODULE/bin/tmux-hub" "tmux-sessions owns the centralized hub command"
assert_contains "$DOTFILES_DIR/home/bin/executable_tmux-hub.tmpl" 'includeTemplate "../modules/tmux-sessions/bin/tmux-hub"' "tmux hub uses a thin tmux-sessions bridge"
...
assert_contains "$TMUX_SESSIONS_TMUX" 'bind-key -N "Open centralized tmux hub" g.*tmux-hub open' "Tmux config opens the centralized hub"
assert_contains "$TMUX_SESSIONS_MODULE/bin/tmux-hub" 'close requires an exact session ID' "Tmux hub closes sessions by immutable ID"
assert_contains "$DOTFILES_DIR/nvim/lua/custom/tmux_hub.lua" 'Tab marks multiple' "Tmux hub exposes multi-session cleanup"
...
assert_contains "$TMUX_CONFIG_SOURCE" 'bind-key -N "Search tmux commands with Telescope" Space display-popup.*tmux-command-palette' "Ctrl-a Space opens the Telescope tmux command palette"
assert_contains "$TMUX_CONFIG_SOURCE" 'S-F7.*display-popup.*tmux-command-palette' "Right Command command search reaches tmux through terminal F19"
assert_contains "$TMUX_SESSIONS_TMUX" 'S-F8.*tmux-hub open' "Right Command hub reaches tmux through terminal F20"
...
assert_not_contains "$TERMINAL_WINDOW_TYPES_WHICH_KEY_CYCLE" "[Aa]writ" "Tmux command center omits Awrit cycling"
assert_not_contains "$TERMINAL_WINDOW_TYPES_WHICH_KEY_NEW" "[Aa]writ" "Tmux command center omits Awrit creation"
...
assert_contains "$SCRATCHPADS_COMMAND" "SCRATCHPAD_SESSION_MANAGER_TMUX_SESSION" "Scratchpads CLI declares the session-manager tmux template"
...
assert_contains "$SCRATCHPADS_COMMAND" "attach_session_manager_tmux_session" "Session-manager scratchpad attaches to its dedicated tmux template"
assert_contains "$SCRATCHPADS_MODULE/targets/skhdrc.tmpl" 'fn - p.*scratchpads open sessions' "Fn+P opens the floating session manager"
...
assert_not_contains "$TERMINAL_WINDOW_TYPES_COMMAND" "[Aa]writ" "Tmux template omits the Awrit window type"
...
assert_contains "$APP_FOCUS_MODULE/bin/app-mru.sh" 'APP_MRU_SUPPRESS_FOCUS_FILE' "App MRU declares its programmatic focus marker"
assert_contains "$APP_FOCUS_MODULE/bin/app-mru.sh" 'app_mru_focus_update_is_suppressed' "App MRU ignores cycle-generated focus signals"
assert_contains "$FOCUS_OPEN_TMUX_WINDOW_COMMAND" 'select((.scratchpad // "") == "")' "Fn tmux window slots exclude scratchpads"
assert_contains "$FOCUS_OPEN_TMUX_WINDOW_COMMAND" 'test("^tmux( · .+)?$")' "Fn tmux window slots require an open tmux terminal"
assert_contains "$FOCUS_OPEN_TMUX_WINDOW_COMMAND" 'sort_by(.id)' "Fn tmux window slots remain creation ordered"
assert_contains "$FOCUS_OPEN_TMUX_WINDOW_COMMAND" 'current_index + 1' "Fn+Tab advances through the same tmux window set"
...
assert_contains "$CREATE_SPACE_COMMAND" "auto|focus|move-window|new-window" "Create-space helper supports automatic, forced, and app-new-window modes"
...
assert_contains "$CREATE_SPACE_COMMAND" 'skhd -k "cmd - n"' "Create-space new-window mode emits the app-local Command+n action"
...
assert_not_contains "$DOTFILES_DIR/home/.chezmoidata.toml" 'chezmoi-git:awrit' "Awrit is absent from bootstrap pins"
assert_not_contains "$DOTFILES_DIR/home/.chezmoidata.toml" '^name = "awrit"$' "Awrit is absent from external projects"
assert_file_missing "$DOTFILES_DIR/home/bin/symlink_awrit.tmpl" "Awrit launcher bridge is absent"
assert_file_exists "$DOTFILES_DIR/home/.chezmoiscripts/run_onchange_before_remove-legacy-awrit-link.sh.tmpl" "run-on-change legacy Awrit launcher cleanup is declared"
