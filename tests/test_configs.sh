...
assert_contains "$SKHDRC" "alt - 0x2B.*scratchpads open codex" "Alt+Comma opens the Codex dotfiles scratchpad"
assert_contains "$SKHDRC" "alt - l.*scratchpads open projects" "Alt+L opens the projects tmux scratchpad"
...
assert_contains "$DOTFILES_DIR/home/dot_tmux.conf" "after-new-session\[50\].*tmux-session-template auto.*session_name" "ordinary new tmux sessions use the standard template"
...
assert_contains "$YABAIRC" "BORDER_COLOR=0xff000000" "JankyBorders lines are black"
