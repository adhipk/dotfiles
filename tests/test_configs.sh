...
TMUX_YAZI_OPENER="$DOTFILES_DIR/modules/tmux-yazi/bin/tmux-yazi-open"
...
assert_contains "$NVIM_INIT" "<leader>sh.*builtin.find_files { hidden = true }" "Leader search hidden includes dotfiles"
assert_contains "$NVIM_INIT" "<leader>sm.*builtin.help_tags.*\[S\]earch \[M\]anual" "Leader search manual opens Neovim help tags"
...
assert_contains "$TMUX_YAZI_HELPER" 'EDITOR="$HOME/bin/tmux-yazi-open"' "Cmd+B Yazi routes files to the session Neovim opener"
assert_contains "$TMUX_YAZI_OPENER" '@dotfiles_window_type' "Yazi opener targets typed Neovim windows"
assert_contains "$DOTFILES_DIR/home/dot_config/yazi/yazi.toml" 'show_hidden = true' "Yazi shows hidden files by default"
assert_contains "$DOTFILES_DIR/home/dot_config/yazi/yazi.toml" 'url = "\*", use = "edit"' "Yazi opens files through its Neovim editor"
