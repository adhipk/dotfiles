...
assert_contains "$SKHDRC" "window --display prev" "Previous-display movement targets yabai"
assert_contains "$SKHDRC" "window --display next" "Next-display movement targets yabai"
...
assert_contains "$DOTFILES_DIR/home/bin/executable_tmux-session-picker" 'exec sesh picker --icons --hide-duplicates --separator-aware --tmux' "session-only helper uses the built-in sesh picker"
assert_contains "$DOTFILES_DIR/home/bin/executable_tmux-sessionizer" 'exec sesh picker --icons --hide-duplicates --separator-aware' "sessionizer uses the built-in sesh picker"
assert_contains "$DOTFILES_DIR/home/bin/executable_tmux-sessionizer-zoxide" 'exec sesh picker --icons --hide-duplicates --separator-aware' "zoxide sessionizer uses the built-in sesh picker"
assert_not_contains "$DOTFILES_DIR/home/bin/executable_tmux-session-picker" 'fzf --' "session-only helper does not rebuild sesh's picker with fzf"
assert_not_contains "$DOTFILES_DIR/home/bin/executable_tmux-sessionizer" 'fzf --' "sessionizer does not rebuild sesh's picker with fzf"
assert_not_contains "$DOTFILES_DIR/home/bin/executable_tmux-sessionizer-zoxide" 'fzf --' "zoxide sessionizer does not rebuild sesh's picker with fzf"
...
assert_contains "$DOTFILES_DIR/home/bin/executable_tmux-session-template" "allows_legacy_index_migration" "tmux protects the new Tuxedo slot during template upgrades"
...
assert_contains "$DOTFILES_DIR/home/bin/executable_scratchpads" "close_duplicate_scratchpad_title_windows" "scratchpads removes same-title unlabeled launch-race windows"
assert_contains "$DOTFILES_DIR/home/bin/executable_scratchpads" "acquire_scratchpad_open_lock" "scratchpads serialize concurrent hotkey launches"
assert_contains "$DOTFILES_DIR/home/bin/executable_scratchpads" 'visible_ids=$(scratchpad_visible_ids' "visible same-target scratchpads toggle closed even after focus moves"
...
assert_contains "$DOTFILES_DIR/home/bin/executable_scratchpads" "window-padding-x=12" "scratchpads add horizontal breathing room"
assert_contains "$DOTFILES_DIR/home/bin/executable_scratchpads" "window-padding-y=10" "scratchpads add vertical breathing room"
assert_contains "$DOTFILES_DIR/home/bin/executable_scratchpads" "window-padding-balance=true" "scratchpads keep terminal padding balanced"
assert_contains "$DOTFILES_DIR/home/bin/executable_scratchpads" "resize-overlay=never" "scratchpads hide resize telemetry"
assert_contains "$DOTFILES_DIR/home/bin/executable_scratchpads" "window-save-state=never" "scratchpads leave geometry ownership to yabai"
...
assert_contains "$DOTFILES_DIR/home/bin/executable_scratchpads" "suppress-scratchpads" "scratchpads remove only their exact JankyBorders window"
