...
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/dotfiles-macos-default-apps-module.XXXXXX")"
TEMP_DIR="$(cd "$TEMP_DIR" && pwd -P)"
DESTINATION="$TEMP_DIR/home"
STATE="$TEMP_DIR/state.db"
EXPECTED_TARGET="$DESTINATION/projects/macos-default-apps/bin/default-apps"
PASSED=0
FAILED=0
...
echo "macos-default-apps module adapter"

assert_file "$MODULE_DIR/module.yaml" "module manifest exists"
assert_file "$MODULE_DIR/targets/default-apps-path.tmpl" "module owns its external path template"
assert_contains "$DOTFILES_DIR/home/bin/symlink_default-apps.tmpl" \
    'if .modules.macosDefaultApps.enabled' "bridge follows module enablement"
assert_contains "$DOTFILES_DIR/home/bin/symlink_default-apps.tmpl" \
    'includeTemplate "../../modules/macos-default-apps/targets/default-apps-path.tmpl"' \
...
printf '%s\n' '#!/usr/bin/env bash' 'printf "%s\\n" default-apps-adapter' >"$EXPECTED_TARGET"
...
if [[ -L "$DESTINATION/bin/default-apps" ]]; then
    pass "enabled profile installs a symlink"
else
    fail "enabled profile installs a symlink" "missing symlink"
fi

actual_target="$(readlink "$DESTINATION/bin/default-apps" 2>/dev/null || true)"
...
actual_output="$("$DESTINATION/bin/default-apps" 2>/dev/null || true)"
[[ "$actual_output" == "default-apps-adapter" ]] \
...
if HOME="$DESTINATION" chezmoi -S "$DOTFILES_DIR" -D "$DESTINATION" --persistent-state "$STATE" \
    --override-data '{"modules":{"macosDefaultApps":{"enabled":false}}}' \
    apply --exclude=scripts,externals --force >/dev/null; then
...
[[ ! -e "$DESTINATION/bin/default-apps" && ! -L "$DESTINATION/bin/default-apps" ]] \
