...
if [[ ! -r "$APP_FOCUS_CONFIG_LIB" ]]; then
    printf 'focus_app.sh: missing app-focus config adapter: %s\n' "$APP_FOCUS_CONFIG_LIB" >&2
    exit 1
fi
...
case "$ARG" in
    @browser)
        ...
        ;;
    @editor)
        APP="${EDITOR_APP:-$(app_focus_config_string '.apps.editor' 'VSCodium')}"
