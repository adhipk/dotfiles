...
# tmux configs
for file in "$DOTFILES_DIR/config/tmux"/*; do
    if [ -f "$file" ]; then
        ln -sf "$file" "$HOME/.config/tmux/$(basename "$file")"
    fi
done
...
# Helper commands (symlink scripts/*.js -> ~/bin/<name>)
for script in "$DOTFILES_DIR/scripts"/*.js; do
    if [ -f "$script" ]; then
        name="$(basename "$script")"
        name="${name%.js}"
        ln -sf "$script" "$HOME/bin/$name"
...
chmod +x "$DOTFILES_DIR/config/tmux"/*.sh 2>/dev/null || true
chmod +x "$DOTFILES_DIR/colorschemes"/*.sh
chmod +x "$DOTFILES_DIR/scripts"/*.sh 2>/dev/null || true
chmod +x "$DOTFILES_DIR/scripts"/*.js 2>/dev/null || true
